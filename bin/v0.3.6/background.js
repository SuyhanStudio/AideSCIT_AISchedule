...
const messageListener = (message, _, sendResponse) => {
  ...
  const cookieHandler = async type => {
    if (type === 'get') {
      chrome.cookies.get({ url: 'https://open-schedule.ai.xiaomi.com/', name: 'userId' }, cookieInfo => {
    ...
    if (type === 'del') {
      await chrome.cookies.getAll({ url: 'https://open-schedule.ai.xiaomi.com/' }, cookieList => {
        for (let cookie of cookieList) {
          chrome.cookies.remove({ url: 'https://open-schedule.ai.xiaomi.com/', name: cookie.name })
        }
        return sendResponse({
          ok: true,
          data: null,
