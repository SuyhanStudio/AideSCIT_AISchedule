 ...
 * @param html 从 scheduleHtmlProvider 方法中获取到的包含课表信息的 HTML 字符串
 * @returns {Array<{
 *      name: string,
 *      weeks: number[],
 *      teacher: string,
            ...
            for (let singleIndex = 0; singleIndex < singleClass.length; singleIndex++){
                ...
                for (let weeksIndex = 0; weeksIndex < weeks.length; weeksIndex++) {
                    ...
                    for (let rangeIndex = localRange[0]; rangeIndex <= localRange[1]; rangeIndex++) {
                        ...
                        // 判断单双周
                        if ((week.indexOf("双") >= 0 && !index)
                            || ((week.indexOf("单") >= 0) && index)) {
                            continue
                        }
                        single.weeks.push(rangeIndex)
                    }
                }
